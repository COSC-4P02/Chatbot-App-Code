var util = {
	options: {
		ACTIVE_COLOR: "#c00", // 按钮颜色
		NORMAL_COLOR: "#000",
		subpages: ["https://web.chatbot-ai.gq/", "html/tab-webview-subpage-contact.html"]
	},
	/**
	 *  简单封装了绘制原生view控件的方法
		...
		for(var i = 0, len = subpages.length; i < len; i++) {
			...
			if(!plus.webview.getWebviewById(subpages[i])) {
				...
				if (i == 0){
					...
					// 	if (!e.target.getURL().includes(subpages[0])){
					// 		e.target.back();
					// 		plus.runtime.openURL(e.target.getURL(), function(res) {  
					// 			console.log(res);  
					// 		});
					// 	}
					// }, false);
